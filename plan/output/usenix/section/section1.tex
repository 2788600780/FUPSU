...
\hline
\textbf{Protocol} & \textbf{Updatable} & \textbf{Fwd. Priv.} & \textbf{UC} & \textbf{Adaptive} & \textbf{Complexity} \\
\hline
...
FUPSI~\cite{WZC+24}      & \checkmark & \checkmark & -- & -- & $O(\log n)$ \\
LMR+26~\cite{LMR+26}     & \checkmark & -- & \checkmark & \checkmark & $O(|\Delta|)$ \\
\hline
\textbf{Ours}            & \checkmark & \checkmark & \checkmark & \checkmark & $O(|\Delta|)$ \\
\hline
\end{tabular}

\smallskip
\caption{Comparison with prior work. $\checkmark$ = property satisfied,
-- = not supported or not proved.
$\dagger$: enhanced security (no within-execution leakage).
FUPSI and LMR+26 are PSI-only; all others are PSU.}
