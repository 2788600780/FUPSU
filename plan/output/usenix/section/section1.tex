...
We implement F-PSU end-to-end in C++ and benchmark across set sizes $2^{14}$
...
updatability. F-PSU's OT queries and delta IBLT sizes reveal these four counts,
...
We implement F-PSU end-to-end in C++ (IBLT with $k{=}5$, OT-masked deletion,
...
\smallskip\noindent
On the security front, we prove F-PSU UC-realizes~\cite{Can01}
...
guarantee of the PRF chain. Because PSU outputs the union, F-PSU is
...
Table~I situates F-PSU among prior PSU and updatable protocols.
...
$\ddagger$: PSI only; $\mathsection$: unified PSO (PSU$+$PSI$+$CA in one execution).
$\checkmark$ = property satisfied; --- = not supported.
F-PSU's adaptive resistance follows from PSU's superset output (see Section~\ref{sec:security}).}
\label{tab:intro-compare}
\begin{tabular}{lccccccc}
\toprule
\textbf{Protocol} & \textbf{Type} & \textbf{Upd.} & \textbf{FW} & \textbf{UC} & \textbf{Adp.} & \textbf{Unif.} & \textbf{Cost/Epoch} \\
\midrule
KRT19~\cite{KRT19} & PSU & --- & --- & \checkmark & --- & --- & $O(n)$ \\
ZCL+23~\cite{ZCL+23} & PSU & --- & --- & \checkmark & --- & --- & $O(n)$ \\
Jia+24~\cite{Jia+24} & PSU & --- & ---$^\dagger$ & --- & --- & --- & $O(n\log n)$ \\
Tu+25~\cite{Tu+25} & PSU & --- & ---$^\dagger$ & \checkmark & --- & --- & $O(n)$ \\
PT26~\cite{PT26} & PSU & --- & --- & \checkmark & --- & --- & $O(n)$ \\
FUPSI~\cite{WZC+24} & PSI$^\ddagger$ & \checkmark & \checkmark & --- & --- & --- & $O(\log n)$ \\
LMR+26~\cite{LMR+26} & PSI$^\ddagger$ & \checkmark & --- & \checkmark & \checkmark & --- & $O(|\Delta|)$ \\
\midrule
\textbf{F-PSU (ours)} & \textbf{PSU} & \textbf{\checkmark} & \textbf{\checkmark} & \textbf{\checkmark} & \textbf{\checkmark} & $\mathsection$ & $\mathbf{O(|\Delta|)}$ \\
\bottomrule
\end{tabular}
\smallskip

\noindent Upd. = updatable; FW = forward privacy; Adp. = adaptive-input resistant; Unif. = unified PSO.
\end{table*}
