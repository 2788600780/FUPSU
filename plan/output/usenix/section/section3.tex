...
\subsection{Epoch Protocol}\label{sec:epoch-protocol}
	...
	\begin{figure*}[t]
\footnotesize
\noindent\textbf{Protocol $\Pi_{\mathsf{fpsu}}$ (Epoch $t \geq 1$)}

\smallskip
\noindent\textbf{Parameters:}
Two parties $P_0, P_1$;
IBLT parameters $\mathsf{prm} = (M, k, \ell, H)$ with $k = 5$;
security parameter $\kappa$;
$\PRF : \{0,1\}^\kappa \times \{0,1\}^* \to \{0,1\}^\kappa$;
$\SE = (\SE.\Enc, \SE.\Dec)$;
ideal functionalities $\cF_{\mathsf{OPRF}}, \cF_{\mathsf{OT}}$.
VOLE state set up once at Epoch~0 and reused across all epochs.

\smallskip
\noindent\textbf{Input:}
Each party $P_b$ ($b \in \{0,1\}$) provides:
encrypted IBLT $\mathsf{CT}_{b}$,
current set $S_{b}$,
key $K_{b}$,
deltas $\Delta^+_{b}, \Delta^-_{b}$.

\smallskip
\noindent\textbf{Protocol:}
\begin{enumerate}
\item \textbf{[Decrypt and prepare.]} \hfill \textit{(local)}
\smallskip

$P_b$ decrypts $\mathsf{IBLT}_{b} \leftarrow \SE.\Dec(K_{b}, \mathsf{CT}_{b})$.
$P_b$ encodes additions into a fresh compact IBLT:
$\mathsf{IBLT}^+_{b} \leftarrow \Encode(\Delta^+_{b})$
with $m_{\Delta} = c_k \cdot k \cdot \max(|\Delta^+_0|, |\Delta^+_1|)$ cells.
\emph{(Do not merge into $\mathsf{IBLT}_{b}$ yet.)}

\item \textbf{[Mini UnionPeel --- additions.]} \hfill \textit{(interactive)}
\smallskip

$P_0$ and $P_1$ run $\Pi_{\mathsf{UnionPeel}}(\mathsf{IBLT}^+_0, \mathsf{IBLT}^+_1)$
(Fig.~\ref{fig:unionpeel}) on all $m_{\Delta}$ cells. The addition IBLT
is fresh---deterministic, zero cascade.
Recover $V_{\Delta^+} \leftarrow \{ v \mid \uPeel(\cdot) \neq \bot \}$.

\item \textbf{[Merge additions.]} \hfill \textit{(local)}
\smallskip

For each $v \in V_{\Delta^+}$:
if $v \in \Delta^+_0 \cap \Delta^+_1$, both parties add $v$ to their IBLT;
if $v \in \Delta^+_b \setminus \Delta^+_{1-b}$, only $P_b$ adds $v$ to $\mathsf{IBLT}_{b}$.

\item \textbf{[OT-masked deletion.]} \hfill \textit{(interactive)}
\smallskip

For each $x \in \Delta^-_b$ (both parties symmetrically):
$P_b$ queries $x$ via batched $\cF_{\mathsf{OPRF}}$.
$P_{1-b}$ responds with $F(k, x)$ and two classification bits:
\[
b_1 \leftarrow \mathbf{1}[x \in S_{1-b}], \qquad
b_2 \leftarrow \mathbf{1}[x \in \Delta^-_{1-b}].
\]
Three cases:
\begin{itemize}
  \item[Case~1] $b_1 = 0$:
    $x$ unique to $P_b$. No IBLT modification.
  \item[Case~2] $b_1 = 1, b_2 = 0$:
    $x$ shared, $P_{1-b}$ retains it.
    $P_b$: $\mathsf{IBLT}_{b} \leftarrow \mathsf{IBLT}_{b} \ominus \Encode(\{x\})$
    ($k$ cell updates, no cascade).
  \item[Case~3] $b_1 = 1, b_2 = 1$:
    both parties independently delete $x$.
    $P_b$: $\mathsf{IBLT}_{b} \leftarrow \mathsf{IBLT}_{b} \ominus \Encode(\{x\})$;
    record modified cells to $Q_{\mathsf{del}}$.
\end{itemize}
In practice, $<2\%$ of deletions fall into Case~3,
eliminating cascade amplification.

\item \textbf{[Resolve Case~3 deletions.]} \hfill \textit{(interactive, conditional)}
\smallskip

If $Q_{\mathsf{del}} \neq \emptyset$: run $\Pi_{\mathsf{UnionPeel}}$ on $Q_{\mathsf{del}}$.
By Lemma~\ref{lem:cascade}, $\leq 2k|\Delta^-_{\mathsf{Case3}}|$ cells examined
w.h.p.\ Recover $V_{\mathsf{del}} \leftarrow \{ v \mid \uPeel(\cdot) \neq \bot \}$.

\item \textbf{[Output.]} \hfill \textit{(local)}
\smallskip

$P_b$ computes:
$S_{\cup} \leftarrow (S_{b} \setminus \Delta^-_{\mathsf{Case1}}) \;\cup\;
\Delta^+_0 \cup \Delta^+_1 \;\cup\; V_{\mathsf{del}}$.
Both parties output $S_{\cup}$.
(Filtering by membership also yields $\mathsf{PSI}$, $\mathsf{PSU}\text{-}\mathsf{CA}$, $\mathsf{PSI}\text{-}\mathsf{CA}$.)

\item \textbf{[Key evolution and re-encryption.]} \hfill \textit{(local)}
\smallskip

$P_b$: $K'_{b} \leftarrow \PRF(K_{b}, \text{``evolve-}b\text{''})$.
$P_b$: $\mathsf{CT}'_{b} \leftarrow \SE.\Enc(K'_{b}, \mathsf{IBLT}_{b})$.
$P_b$ securely erases $K_{b}$.
\end{enumerate}

\caption{The F-PSU incremental-epoch protocol ($t \geq 1$). Steps~1--3 prepare
additions via Mini UnionPeel ($O(|\Delta^+|)$ cells, deterministic).
Steps~4--5 execute OT-masked deletion: deletions are classified into
three cases via a single batched OPRF round, with only Case~3
($<2\%$) entering the IBLT cascade path. Steps~6--7 finalize the
output and evolve keys for forward privacy ($<1$~ms overhead).}
\label{fig:protocol}
\end{figure*}
