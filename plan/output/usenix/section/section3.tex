...
\hline
\textbf{Protocol} & \textbf{Output} & \textbf{Update} & \textbf{Communication} & \textbf{Computation} \\
\hline
\end{tabular}
...
Ours (Epoch $>$0)  & Two-Sided & Add \& Del & $O(d(\sigma+\kappa) + n\sigma)$ & $O(d)$ OT + $O(n)$ $\SE$ \\
\hline
\end{tabular}
...
per-epoch cost proportional to update size $d$.}
