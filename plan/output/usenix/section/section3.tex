...
\subsection{Ideal Functionality}\label{sec:ideal}
	...
	\begin{enumerate}
	\item Upon receiving $(\text{Update}, \Delta^+_b, \Delta^-_b)$ from $P_b$:
	assert $\Delta^+_b \cap X_b = \emptyset$ and $\Delta^-_b \subseteq X_b$;
	...
	\item Upon adaptive corruption of $P_b$ at epoch $\tau$: output
	$X_b^{(\tau)}$ to $\cS$. Historical states $\{X_b^{(t)}\}_{t < \tau}$ are
	not revealed.
	\end{enumerate}
	\smallskip\noindent\textbf{Leakage:} At step~2, the simulator $\cS$ additionally learns
	$\cL = (|\Delta^+_0|, |\Delta^-_0|, |\Delta^+_1|, |\Delta^-_1|)$.
	\smallskip
	\end{minipage}}
