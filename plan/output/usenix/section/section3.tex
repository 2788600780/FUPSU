...
(Fig.~\ref{fig:f-fupsu}) and UC definitions appear in
...
FUPSU leaks these four counts through OT query counts and delta IBLT sizes.
The proof uses a Shannon source-coding argument. FUPSU leaks exactly these
...
$\Pi_{\mathsf{fupsu}}$ already extracts the corrupted party's effective input
...
\subsection{Epoch Protocol}\label{sec:epoch-protocol}
	...
	\smallskip\noindent\textbf{OT-masked deletion.}
	The critical insight of FUPSU is that deletions need not trigger IBLT
...
\footnotesize
\smallskip
\noindent\textbf{Protocol: FUPSU Incremental Epoch ($t \geq 1$)}
...
\end{cases}
\]
The symmetric check runs for $P_{1-b}$'s deletions.
...
\smallskip
\end{minipage}}
\caption{The FUPSU incremental-epoch protocol.}
\label{fig:protocol}
\end{figure*}

\subsection{Complexity}\label{sec:complexity}

\textbf{Unbalanced settings.} FUPSU operates identically in balanced and
...
\hline
\textbf{Protocol} & \textbf{Output} & \textbf{Update} & \textbf{Comm.} & \textbf{Comp.} \\
\hline\noalign{\vspace{-1pt}}\hline
KRT19~\cite{KRT19} & Two-Sided & --- & $O(n\sigma\kappa)$ & $O(n\log n)$ OT \\
PT26~\cite{PT26}   & Two-Sided & --- & $O(n(\sigma+\kappa))$ & $O(n)$ OT + hash \\
Tu+25~\cite{Tu+25} & Two-Sided & --- & $O(n\sigma)$ & $O(n)$ symm. \\
\hline\noalign{\vspace{-1pt}}\hline
Ours (Epoch~0)     & Two-Sided & --- & $O(n(\sigma+\kappa))$ & $O(n)$ OT + hash \\
\hline\noalign{\vspace{-1pt}}\hline
Ours (Epoch $>$0)  & Two-Sided & Add & $O(|\Delta^+|(\sigma+\kappa))$ & $O(|\Delta^+|)$ OT \\
                   &           & Del & $O(|\Delta^-|\kappa)$          & $O(|\Delta^-|)$ OT \\
\hline
\end{tabular}

\caption{Asymptotic complexity of PSU protocols.
$n = |X|+|Y|$, $d = |\Delta|$.
Ours (Epoch~0) matches PT26; Epoch $>$0 achieves
per-epoch cost $O(|\Delta|(\sigma+\kappa) + n\sigma)$ via MiniPeel (Add)
and OT-masked deletion (Del). The union output $n\sigma$ contributes
to both Add and Del communication.}
