...
\subsection{Ideal Functionality}\label{sec:ideal}
	...
	\smallskip
	\end{minipage}}
\captionof{figure}{The ideal functionality of forward-private updatable Private Set Union.}
\label{fig:f-fpsu}
...
\caption{The F-PSU incremental-epoch protocol. Per-epoch cost:
$O(|\Delta^+|)$ cells (MiniPeel, Steps~2--3)
$+$ $O(|\Delta^-|)$ OT checks (Step~4)
$+$ $O(|\Delta^-_{\mathsf{Case3}}|)$ cells (Case~3 cleanup, Step~5).
Forward privacy via key evolution (Step~6): $<1$~ms overhead.}
\label{fig:protocol}
\smallskip
\end{minipage}}
\end{figure*}
