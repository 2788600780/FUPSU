...
\smallskip
\noindent\textbf{Input:}
Each $P_b$ ($b \in \{0,1\}$) provides
$\mathsf{CT}_{b}$, $S_{b}$, $K_{b}$, $\Delta^+_{b}, \Delta^-_{b}$.

\smallskip
\noindent\textbf{Protocol:}\\[-4pt]
\begin{enumerate}
\item \textbf{[Decrypt and prepare additions.]} \textit{[L]}\\[-2pt]
$P_b$ decrypts $\mathsf{IBLT}_{b} \leftarrow \SE.\Dec(K_{b}, \mathsf{CT}_{b})$;
encodes $\mathsf{IBLT}^+_{b} \leftarrow \Encode(\Delta^+_{b})$
with $m_{\Delta} = c_k \cdot k \cdot \max(|\Delta^+_0|, |\Delta^+_1|)$ cells.
(Defer merge into $\mathsf{IBLT}_{b}$.)

\item \textbf{[Mini UnionPeel.]} \textit{[I]}\\[-2pt]
$P_0$ and $P_1$ run $\Pi_{\mathsf{UnionPeel}}(\mathsf{IBLT}^+_0, \mathsf{IBLT}^+_1)$
(Fig.~\ref{fig:unionpeel}) on all $m_{\Delta}$ cells.
Fresh IBLT --- deterministic, zero cascade.
Recover $V_{\Delta^+} \leftarrow \{ v \mid \uPeel(\cdot) \neq \bot \}$.

\item \textbf{[Merge.]} \textit{[L]}\\[-2pt]
For each $v \in V_{\Delta^+}$:
$v \in \Delta^+_0 \cap \Delta^+_1 \Rightarrow$ both add to IBLT;
$v \in \Delta^+_b \setminus \Delta^+_{1-b} \Rightarrow$ only $P_b$ adds.

\item \textbf{[OT-masked deletion.]} \textit{[I]}\\[-2pt]
For each $x \in \Delta^-_b$ (both parties symmetrically):
$P_b$ queries $x$ via batched $\cF_{\mathsf{OPRF}}$.
$P_{1-b}$ responds with classification bits
$b_1 = \mathbf{1}[x \in S_{1-b}]$,
$b_2 = \mathbf{1}[x \in \Delta^-_{1-b}]$, and acts by case:
\[
\begin{cases}
b_1 = 0              & \text{(Case~1) unique to } P_b \text{ --- no IBLT touch.} \\
b_1 = 1,\; b_2 = 0   & \text{(Case~2) shared, peer retains --- } P_b \text{ subtracts } \Encode(\{x\}). \\
b_1 = 1,\; b_2 = 1   & \text{(Case~3) both delete --- } P_b \text{ subtracts } \Encode(\{x\}), \text{ record cells to } Q_{\mathsf{del}}.
\end{cases}
\]

\item \textbf{[Resolve Case~3.]} \textit{[I]}\\[-2pt]
If $Q_{\mathsf{del}} \neq \emptyset$: run $\Pi_{\mathsf{UnionPeel}}$ on $Q_{\mathsf{del}}$
($\leq 2k|\Delta^-_{\mathsf{Case3}}|$ cells by Lemma~\ref{lem:cascade}).
Recover $V_{\mathsf{del}} \leftarrow \{ v \mid \uPeel(\cdot) \neq \bot \}$.

\item \textbf{[Output and key evolution.]} \textit{[L]}\\[-2pt]
$P_b$ outputs
$S_{\cup} \leftarrow (S_{b} \setminus \Delta^-_{\mathsf{Case1}}) \cup
\Delta^+_0 \cup \Delta^+_1 \cup V_{\mathsf{del}}$
(by membership filtering, also $\mathsf{PSI}$, $\mathsf{PSU}\text{-}\mathsf{CA}$, $\mathsf{PSI}\text{-}\mathsf{CA}$).
$P_b$: $K'_{b} \leftarrow \PRF(K_{b}, \text{``evolve-}b\text{''})$,
$\mathsf{CT}'_{b} \leftarrow \SE.\Enc(K'_{b}, \mathsf{IBLT}_{b})$,
$\mathsf{secure\_erase}(K_{b})$.
\end{enumerate}

\caption{The F-PSU incremental-epoch protocol ($t \geq 1$).
Per-epoch cost: $O(|\Delta^+|)$ cells (MiniPeel, Steps~2--3)
$+$ $O(|\Delta^-|)$ OT checks (Step~4)
$+$ $O(|\Delta^-_{\mathsf{Case3}}|)$ cells (Case~3 cleanup, Step~5).
Forward privacy: $<1$~ms overhead (Step~6). Notation: [L] = local, [I] = interactive.}
\label{fig:protocol}
\end{figure*}
