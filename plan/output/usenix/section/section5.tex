...
We validate FUPSU via an end-to-end C++ implementation on an Intel Xeon
...
\hline
$n$ & \textbf{Protocol} & $|\Delta|$ & \textbf{Comm. (MB)} &
...
& FUPSU (Epoch~0)    & --- & 32.1  & 0.13s  & 1.78s  & 5.71s  & 52.9s \\
\cline{2-8}
& FUPSU (Epoch~$>$0) & 256  & 0.92  & 0.02s  & 0.10s  & 0.11s  & 0.21s \\
& FUPSU (Epoch~$>$0) & 1024 & 1.12  & 0.02s  & 0.11s  & 0.14s  & 0.56s \\
& FUPSU (Epoch~$>$0) & 4096 & 1.94  & 0.02s  & 0.15s  & 0.29s  & 1.94s \\
\hline\noalign{\vspace{-1pt}}\hline
\multirow{8}{*}{$2^{18}$}
...
& FUPSU (Epoch~0)    & --- & 128   & 0.66s  & 6.18s  & 21.9s  & 210.7s \\
\cline{2-8}
& FUPSU (Epoch~$>$0) & 256  & 3.47  & 0.07s  & 0.15s  & 0.16s  & 0.26s \\
& FUPSU (Epoch~$>$0) & 1024 & 3.67  & 0.07s  & 0.17s  & 0.20s  & 0.61s \\
& FUPSU (Epoch~$>$0) & 4096 & 4.49  & 0.08s  & 0.21s  & 0.35s  & 2.00s \\
\hline\noalign{\vspace{-1pt}}\hline
\multirow{8}{*}{$2^{20}$}
...
& FUPSU (Epoch~0)    & --- & 514   & 3.93s  & 25.2s  & 88.1s  & 843.1s \\
\cline{2-8}
& FUPSU (Epoch~$>$0) & 256  & 13.7  & 0.44s  & 0.52s  & 0.53s  & 0.64s \\
& FUPSU (Epoch~$>$0) & 1024 & 13.9  & 0.43s  & 0.52s  & 0.55s  & 0.97s \\
& FUPSU (Epoch~$>$0) & 4096 & 14.7  & 0.40s  & 0.53s  & 0.66s  & 2.32s \\
\hline
\end{tabular}
\caption{End-to-end protocol performance. Static baselines as reported
in~\cite{PT26} (AMD EPYC 74F3, single-threaded). FUPSU measured on Intel
...
\smallskip\noindent
At $|\Delta| = 2^{10}$, FUPSU epoch~$>$0 communication is constant at
...
FUPSU is the only protocol supporting updatable execution with forward
...
Table~I in the Introduction situates FUPSU among prior PSU and updatable
...
indicators per day), FUPSU exchanges $\approx$0.5~KB of IBLT data per daily
