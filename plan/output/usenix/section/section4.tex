...
We prove that $\Pi_{\mathsf{fupsu}}$ UC-realizes $\cF_{\mathsf{updPSU}}[f_{\cup}]$
(Fig.~\ref{fig:f-fupsu}) in the $(\cF_{\mathsf{OPRF}}, \cF_{\mathsf{OT}})$-hybrid
...
$\Pi_{\mathsf{fupsu}}$ UC-realizes $\cF_{\mathsf{updPSU}}[f_{\cup}]$ in the
...
If $\Pi_{\mathsf{fupsu}}$ UC-realizes $\cF_{\mathsf{updPSU}}$ with adaptive
...
show that FUPSU requires no modification to resist this class of attacks.
...
\smallskip\noindent\textbf{Why FUPSU is immune.}
FUPSU's state has two layers of protection:
\begin{enumerate}
...
functionality $\cF_{\mathsf{updPSU}}$ (Fig.~\ref{fig:f-fupsu}). Adaptive choice
...
The OT-based membership check in Step~4 of $\Pi_{\mathsf{fupsu}}$ (Fig.~\ref{fig:protocol})
...
with the per-epoch UC proof (Theorem~\ref{thm:main}), the full FUPSU protocol
